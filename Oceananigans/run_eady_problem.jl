...
grid = RegularCartesianGrid(size=(192, 192, 64), x=(0, 4e6), y=(0, 4e6), z=(-4e3, 0))

coriolis = FPlane(f=1e-4) # [s⁻¹]
                            
background_parameters = ( α = 1.0 * coriolis.f, # s⁻¹, geostrophic shear
...
max_Δt = min(0.5 * hour, grid.Δx / Ū, 0.5 * grid.Δx^4 / κ₄h, 0.5 * grid.Δz^2 / κ₂z)

cfl = 1.5 * min(1.5, drag_coefficient * grid.Δx / grid.Δz)
